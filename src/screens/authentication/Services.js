...
const Services = props => {
 ...
 const submit = async() => {
   const {name,email,phone,address,zipCode,country,lat,long,category, type, id, images} = props.route.params.data; 
   console.log( images.length, ownerId, name, category, id , type, address, {location: {"type": "Point", "coordinates": [lat, long]}} , shopServices);
    await props.register_shop({ owner:ownerId, title:name, work_type:category, plan:id , shop_type:type, address, location: {"type": "Point", "coordinates": [long, lat]}, images, services:shopServices,country, zip_code:zipCode})
