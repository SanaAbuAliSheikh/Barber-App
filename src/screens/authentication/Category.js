...
const Category = props => {
  ...
  const onSubmit = async () => {
    ...
    if (no_of_shops != 0 && shopId) {
      ...
      console.log(shopExist);
      const redirect = 'Shop Info';
      props.navigation.navigate(redirect, {
        type: barber[0].value == 1 ? 'barber' : 'beauty',
        category: shop[0].value == 1 ? 'physical' : 'remote',
      });
    } else {
      setShopExist(false);
      const redirect = 'Package Plan';
      props.navigation.navigate(redirect, {
        type: barber[0].value == 1 ? 'barber' : 'beauty',
        category: shop[0].value == 1 ? 'physical' : 'remote',
      });
  ...
  return (
    <View style={Styles.background}>
          ...
          <View style={{flexDirection: 'row', justifyContent: 'center'}}>
            <View style={{flexDirection: 'row', margin: 20}}>
              <RadioButton
                ...
                onPress={value => {
                  setShop([{label: '', value: 0}]),
                    setRemote([{label: '', value: 1}]);
                }}
                formHorizontal={true}
            ...
            <View style={{flexDirection: 'row', margin: 20}}>
              <RadioButton
                ...
                onPress={value => {
                  setShop([{label: '', value: 1}]),
                    setRemote([{label: '', value: 0}]);
                }}
                formHorizontal={true}
                labelStyle={{fontSize: 20}}
                labelHorizontal={true}
            ...
            <View style={{alignItems: 'center', margin: 20}}>
              ...
              <View style={{position: 'absolute', right: 10}}>
                <RadioButton
                  ...
                  onPress={value => {
                    setBeauty([{label: '', value: 0}]),
                      setBarber([{label: '', value: 1}]);
                  }}
                  formHorizontal={true}
                  labelStyle={{fontSize: 20}}
                  labelHorizontal={true}
            ...
            <View style={{alignItems: 'center', margin: 20}}>
              ...
              <View style={{position: 'absolute', right: 10}}>
                <RadioButton
                  ...
                  onPress={value => {
                    setBeauty([{label: '', value: 1}]),
                      setBarber([{label: '', value: 0}]);
                  }}
                  formHorizontal={true}
                  labelStyle={{fontSize: 20}}
                  labelHorizontal={true}
      ...
      {(shop[0].value == 0 && remote[0].value == 0) ||
      (barber[0].value == 0 && beauty[0].value == 0) ? (
